...
function init_env() {
  ...
  export INPUT_NOTIFY_SUFFIX="${INPUT_NOTIFY_SUFFIX:-Powered by https://github.com/ale-novo/mirror}"
...
function mirror() {
  ...
  for repo_name in ${INPUT_MIRROR_REPOS}; do
    ...
    if ! git clone -q --bare "${source_addr}" "${REPO_NAME}"; then
